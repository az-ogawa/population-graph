...
export const MainContents: FC = memo(() => {
  ...
  useEffect(() => {
    ...
    selectedPrefectures.forEach((selectedPrefecture) => {
      const prefName =
        prefectures &&
        prefectures.result.find((pref) => pref.prefCode === selectedPrefecture)
          ?.prefName;
      const selectedData: SelectedPopulationData = {
        data: populationData,
        prefCode: selectedPrefecture,
        prefName: prefName !== undefined && prefName !== null ? prefName : "",
      };
  ...
  return (
    <main>
      ...
      <PopulationTypeSelectArea
        populationTypes={populationData.result.data.map(
